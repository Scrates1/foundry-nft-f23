...
contract MoodNftTest is Test {
    ...
    function testViewTokenURI() public {
        vm.prank(USER); // 注意此处需要模拟一个用户。因为NFT是被用户持有的，否则会遇到ERC721InvalidReceiver错误
        moodNft.mintNft();
        console.log(moodNft.tokenURI(0));
        vm.prank(USER);
        moodNft.flipMood(0);
        console.log(moodNft.tokenURI(0)); // 这是哭脸json的uri

        // 为什么如下代码，输出的还是笑脸？1不是没有mintNft吗？ 因为没有，所以s_tokenIdToMood[tokenId]得到的结果是0==>笑脸
